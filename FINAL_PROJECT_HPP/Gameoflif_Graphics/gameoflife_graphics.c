...
void DrawCell(int i, int j, int N, int L, int W ,int  cellColor){
...
void generation_update_draw_cell(cell* cell , int i ,int j , int W , int L , int N ,int  cellColor){
  cell->current = cell->next;
  if (cell->current){
    DrawCell( i , j , N , L , W , cellColor);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  for (int k = 0 ; k < n_steps ; k++){
    ...
    for (int i = 0 ; i < N ; i++){
      for (int j = 0 ; j < M ; j++){
            //int cellColor = rand() % 15;
          generation_update_draw_cell(&cells_grid[i][j], i , j,  W, L , N , cellColor);
...
inline void evolution(cell * cell){
  ...
  if ((counter ==3) || (counter == 2 && (*cell).current ==1)){
    (*cell).next = 1;
    }
  else{
    (*cell).next = 0;
